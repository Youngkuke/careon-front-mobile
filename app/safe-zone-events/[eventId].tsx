...
export default function SafeZoneEventScreen() {
  ...
  useEffect(() => {
    ...
    if (!Number.isInteger(id) || id <= 0) { setLoading(false); return; }
    let cancelled = false;
    void getSafeZoneEvent(id).then((result) => { if (!cancelled) setEvent(result); }).catch(() => {}).finally(() => { if (!cancelled) setLoading(false); });
  ...
  if (!event) return <Screen contentStyle={styles.empty}><Header onBack={() => replaceRoute('/wear')} style={styles.header} title="안심 구역 이탈" /><Text style={styles.emptyText}>이탈 정보를 불러오지 못했어요.</Text></Screen>;
  ...
  return <Screen scroll backgroundColor={CAREON_COLORS.page} contentStyle={styles.content}>
    ...
    <Text onPress={openMap} style={styles.mapLink}>지도에서 보기</Text>
  </Screen>;
}
