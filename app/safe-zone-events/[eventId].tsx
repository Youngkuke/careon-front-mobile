...
export default function SafeZoneEventScreen() {
  ...
  if (loading) return <Screen contentStyle={styles.screen}><Header onBack={() => replaceRoute('/wear')} style={styles.header} title="안심 구역 이탈" /><View style={styles.centeredContent}><ActivityIndicator color={CAREON_COLORS.primaryDark} /></View></Screen>;
  if (!event) return <Screen contentStyle={styles.screen}><Header onBack={() => replaceRoute('/wear')} style={styles.header} title="안심 구역 이탈" /><View style={styles.centeredContent}><Text style={styles.emptyText}>{notFound ? '요청한 안심 구역 이탈 정보를 찾을 수 없어요.' : loadFailed ? '이탈 정보를 불러오지 못했어요.' : '이탈 정보가 없어요.'}</Text>{loadFailed ? <CareButton onPress={() => void loadEvent()} style={styles.retry} variant="white">다시 시도</CareButton> : null}</View></Screen>;
