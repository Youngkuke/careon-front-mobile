...
export default function TrackingScreen() {
  const { liveLocation, liveLocationTrackingEnabled, safeZone, setLiveLocationTracking } = useWear();
  ...
  const toggleTracking = async () => {
    setIsUpdating(true);
    try { await setLiveLocationTracking(!liveLocationTrackingEnabled); }
        ...
        <View style={styles.mapCard}>
          <View style={styles.mapArea}>
            <MapView
              ...
              {safeZone.enabled ? (
                <Circle
                  center={center}
                  fillColor="rgba(50, 190, 145, 0.14)"
          ...
          <View style={styles.mapCaption}>
            ...
            <View style={styles.captionTextWrap}>
              <Text style={styles.captionTitle}>CareOn 워치</Text>
              <Text style={styles.captionText}>{liveLocationTrackingEnabled ? (liveLocation ? '워치의 최신 위치를 표시하고 있어요' : '워치 위치를 기다리고 있어요') : '추적을 시작하면 위치를 갱신해요'}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  title: { color: CAREON_COLORS.title, fontSize: 24, fontWeight: '900' },
  ...
  locateButton: { alignItems: 'center', backgroundColor: CAREON_COLORS.background, borderRadius: 28, bottom: 16, elevation: 5, height: 56, justifyContent: 'center', position: 'absolute', right: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.18, shadowRadius: 7, width: 56 },
  locateButtonDisabled: { backgroundColor: '#F2F2F2', elevation: 0, shadowOpacity: 0 },
  captionIcon: { alignItems: 'center', backgroundColor: '#E7F8F2', borderRadius: 16, height: 36, justifyContent: 'center', width: 36 },
  ...
  status: { backgroundColor: '#FFF0D9', borderRadius: 12, paddingHorizontal: 9, paddingVertical: 5 },
  statusActive: { backgroundColor: '#DDF8EF' },
  statusText: { color: '#B86B00', fontSize: 11, fontWeight: '800' },
  ...
  divider: { backgroundColor: '#F1F1F1', height: 1 },
